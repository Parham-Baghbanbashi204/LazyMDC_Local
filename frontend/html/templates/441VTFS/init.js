...
var re_mgrs = new RegExp(
  [/^([0-9]+[A-Z])/, /([A-Z][A-Z])/, /([0-9]+)/].map((r) => r.source).join(""),
);
...
function get_brg(from_lat, from_lon, to_lat, to_lon, kmh = false) {
  ...
  return [
    Math.round(azi).toString().padStart(3, "0"),
    Math.round(distance).toFixed(0),
  ];
}
...
function display_coord(data, info, split_mgrs = false) {
  ...
  var coord =
    info["lat_fmt"] === undefined
      ? data.flight["flight-coord"]
      : info["lat_fmt"];
  var decimals = parseInt(
    info["lat_dmp"] === undefined
      ? data.flight["flight-coord-decimals"]
      : info["lat_dmp"],
  );
  ...
  return [ll(data, info, true), ll(data, info, false)];
}
...
function ll(data, info, lat) {
  ...
  if (coord == "ddm") {
    ...
    return `${axis}${deg.toString().padStart(pad)}&deg;${min
      .toFixed(decimals)
      .padStart(dec_width, 0)}`;
  }
  ...
  var sec =
    Math.round((work + Number.EPSILON) * 10 ** decimals) / 10 ** decimals;
...
function alt_formatter(data, alt) {
  ...
  if (alt_i > trans) return `FL${alt_i / 100}`;
  ...
  this.dom = (function () {
    ...
    html += `
          ...
          <tr>
            ...
            <td class="text-center text-bold rb0">${
              data["mission"]["mission-pri-freq"]
                ? data["mission"]["mission-pri-freq"].value || ""
                : ""
            }</td>
            <td class="lp5 lb0">${
              data["mission"]["mission-pri-freq"]
                ? data["mission"]["mission-pri-freq"].pst || ""
                : ""
            }</td>
            <td class="text-center text-bold rb0">${
              data["mission"]["mission-sec-freq"]
                ? data["mission"]["mission-sec-freq"].value || ""
                : ""
            }</td>
            <td class="lp5 lb0">${
              data["mission"]["mission-sec-freq"]
                ? data["mission"]["mission-sec-freq"].pst || ""
                : ""
            }</td>`;

    if (package) {
      html += `
            <td class="text-center overflow-hidden">${data["package"][
              "package-name"
            ].toUpperCase()}</td>`;
    }
...
function Flight(data, unit) {
  ...
  var cols = (function () {
    ...
    if (
      data["flight"]["members"][0] &&
      data["flight"]["members"][0]["mids a"] !== undefined
    ) {
    ...
    if (
      data["flight"]["members"][0] &&
      data["flight"]["members"][0]["mids b"] !== undefined
    ) {
    ...
    if (
      data["flight"]["members"][0] &&
      data["flight"]["members"][0]["hm device"] !== undefined
    ) {
    ...
    if (
      data["flight"]["members"][0] &&
      data["flight"]["members"][0]["lsr"] !== undefined
    ) {
    ...
    if (
      data["flight"]["members"][0] &&
      data["flight"]["members"][0]["lss"] !== undefined
    ) {
    ...
    for (var member of data["flight"]["members"]) {
      ...
      for (var col of cols) {
        ...
        if (col[0] == "#") {
          if (data["mission"]["mission-flight-number"]) {
            html +=
              data["mission"]["mission-flight-number"] + "-" + member["id"];
          } else {
            html += member["id"];
          }
        } else {
...
function Package(data, unit) {
  ...
  this.content = (function () {
    ...
    for (var member of data["package"]["members"]) {
      var non_empty = Object.values(member).filter(function (x) {
        return x;
      });
      if (!non_empty.length) {
        continue;
...
function F16CMDS(data, unit) {
  ...
  this.table = function () {
    ...
    for (const col in modes) {
      ...
      head += `<th colspan=2 class="text-center text-bottom${
        col != cols - 1 ? " rb2" : ""
      }">${title}</th>`;
    }
    ...
    ["BQ", "BI", "SQ", "SI"].forEach(function (param) {
      ...
      for (var idx in modes) {
        ["CHAFF", "FLARE"].forEach(function (cms) {
          ...
          row += `<td class="${cls}">${format(
            data.profiles.cmds[mode][cms][param],
            param == "SI" ? 3 : 2,
          ).replace(/  /g, "&nbsp;")}</td>`;
        });
      }
...
function F16Harm(data, unit) {
  ...
  this.table = function () {
    ...
    for (var table = 0; table < 3; table++) {
      ...
      html += `<th colspan=3 class="text-center${
        table == 2 ? "" : " rb2"
      }">${title}</th>`;
    }
    ...
    for (var row = 0; row < 5; row++) {
      ...
      for (var col = 0; col < 8; col++) {
        if (row == 0 && col < 6) {
          ...
          let content = "-";
          if (
            data.profiles.hts.hasOwnProperty("man") &&
            data.profiles.hts.man.length > tableIdx
          ) {
            content = data.profiles.hts.man[tableIdx][attr];
          }

          var cls = attr === "name" ? "lp5 rb2 text-harm" : "text-center";

          html += `<td class="${cls}">${content}</td>`;
        } else {
          let classId = row + 1 + (col == 7 ? 5 : 0);
          const on =
            !data.profiles.hts.hasOwnProperty("classes") ||
            data.profiles.hts.classes.includes(classId);
...
var Loadout = function (data, unit) {
  ...
  var clsid_renames = {
    ...
    "{40EF17B7-F508-45de-8566-6FFECC0C1AB8}": "AIM-120C AMRAAM",
  ...
  this.content = [
    (function () {
      ...
      other_col.push([
        "FUEL",
        ntos(data["loadout"]["fuel"]) + "%",
        "text-bold",
      ]);

      if (data.loadout.chaff) {
        other_col.push(["CHAFF", ntos(data.loadout.chaff), ""]);
      }
      if (data.loadout.flare) {
        other_col.push(["FLARE", ntos(data.loadout.flare), ""]);
      }
      // if (data.loadout.tiger && data.loadout.tiger !== "0") { other_col.push(['TIGER', ntos(data.loadout.tiger), ""]); };

      var joker_bingo = [];
      if (data.loadout.joker) {
        joker_bingo.push(["JOKER", ntos(data.loadout.joker), ""]);
      }
      if (data.loadout.bingo) {
        joker_bingo.push(["BINGO", ntos(data.loadout.bingo), ""]);
      }
      ...
      var loop_count = Math.max(
        pylons_count,
        other_col.length,
        lbs_col.length + mtow_rows,
      );
      ...
      for (var n = 0; n < loop_count; n++) {
        ...
        var joker_bingo_itm = joker_bingo[n];

        html += `
        <tr style="border:0">`;

        if (pyl_name) {
          // changed nowrap to wrap to see if it will wrap the store
          html += `
        ...
        // --- OTHER column (2 cols) ---
        // Print OTHER rows first, then J/B header + JOKER/BINGO underneath, all inside SAME 2 columns.

        var otherLen = other_col.length;
        var jbLen = joker_bingo.length;

        // Row indices for the J/B block within the SAME OTHER columns
        // J/B header goes immediately after the last OTHER row
        var jbHeaderRow = otherLen; // first row AFTER OTHER
        var jbFirstDataRow = jbHeaderRow + 1; // JOKER starts here
        var jbEndRow = jbFirstDataRow + jbLen - 1; // last J/B data row

        if (n < otherLen && other_col[n]) {
          // Normal OTHER rows (GUN/FUEL/CHAFF/FLARE)
          var other_itm = other_col[n];
          html += `
          <td class="lp5">${other_itm[0]}</td>
          <td class="text-center rb2 ${other_itm[2]}">${other_itm[1]}</td>`;
        } else if (jbLen && n === jbHeaderRow) {
          // J/B header row directly below FLARE (or last OTHER)
          html += `<th class="jbheader" colspan="2">J/B</th>`;
        } else if (jbLen && n >= jbFirstDataRow && n <= jbEndRow) {
          // JOKER/BINGO rows under the header
          var jb_itm = joker_bingo[n - jbFirstDataRow];
          html += `
          <td class="lp5">${jb_itm[0]}</td>
          <td class="text-center rb2 ${jb_itm[2] || ""}">${jb_itm[1]}</td>`;
        } else {
          // Empty filler cells in OTHER area
        ...
        if (n == loop_count - mtow_rows) {
          ...
          if (data.loadout.weights.mtow_cvn) {
            ...
            <td class="text-right rp5">${ntos(
              data["loadout"]["weights"]["mtow_field"],
            )}</td>
            <td class="text-right rp5">${ntos(
              data["loadout"]["weights"]["mtow_cvn"],
            )}</td>`;
          } else if (data.loadout.weights.mtow_vtol) {
            html += `
            ...
            <td class="text-right rp5">${ntos(
              data["loadout"]["weights"]["mtow_vtol"],
            )}</td>`;
          } else {
            html += `<th class="text-center">${
              single_max ? "MAX" : "MAX T/O"
            }</th>
                    <td colspan=2 class="text-right rp5">${ntos(
                      data["loadout"]["weights"]["mtow_field"],
                    )}</td>`;
          }
          ...
          if (data.loadout.weights.mtow_cvn) {
            html += `
            <td class="text-right rp5">${ntos(
              data["loadout"]["weights"]["mlw_field"],
            )}</td>
            <td class="text-right rp5">${ntos(
              data["loadout"]["weights"]["mlw_cvn"],
            )}</td>`;
          } else {
            html += `
            <td colspan=2 class="text-right rp5">${ntos(
...
var Tacans = function (data, unit) {
  ...
  this.content = (function () {
    ...
    for (const tcn of Object.keys(data.mission["mission-beacons"] || {}).sort(
...
var Presets = function (data, unit) {
  ...
  // ---- PATCH: display-only SEC -> AUX ----
  const secToAux = (v) => {
    if (v === null || v === undefined) return v;
    if (typeof v !== "string") return v;
    return v.replace(/\bSEC\b/gi, "AUX");
  };
  const radioLabel = (radioKey) => secToAux(String(radioKey).toUpperCase());
  // ---------------------------------------
  ...
  this.table = function () {
    ...
    var priorities =
      data.priority.length > 0 ? data.priority : Object.keys(data.radios);

    if (["AH-64D", "C-130J-30"].includes(ac)) {
      // handled in content()
    } else if (priorities.length > 0) {
      var col_group =
        "<col width=40px /><col width=85px/><col width=250px/><col width=18px/>".repeat(
          priorities.length - 1,
        );

      var headers = "";
      for (const idx in priorities) {
        // ---- PATCH: header display label ----
        headers += `<th colspan=3>${radioLabel(priorities[idx])}</th>`;
        // ------------------------------------
  ...
  this.content = (function () {
    ...
    if (["AH-64D", "C-130J-30"].includes(ac)) {
      var priorities = data.priority.length
        ? data.priority
        : Object.keys(data.radios);
      for (var start of [0, 2]) {
        ...
        <table class="kb-width std std-striped" style="table-layout: fixed">
          ...
          <tbody>
            <tr class="header">
              <!-- PATCH: display-only label -->
              <th colspan=3>${radioLabel(priorities[start])}</th>
              <td class="bg-blank" style="border:0"></td>
              <th colspan=3>${radioLabel(priorities[start + 1])}</th>
            </tr>`;
        ...
        for (var x = 1; x <= presets; x++) {
          ...
          html += `
            ...
            <!-- PATCH: SEC->AUX in override and name -->
            <td class="text-bold text-right rp5">${
              pst.override ? secToAux(pst.override) : pst.value
            }</td>
            <td class="lp5">${secToAux(name)}</td>
            <td class="bg-blank" style="border:0"></td>
          `;
      ...
      var presets = Math.max(
        ...Object.values(data.radios).map((x) => Object.keys(x).length),
      );
      ...
      for (var x = 1; x <= presets; x++) {
        ...
        for (const [radio_id, radio] of Object.entries(priorities)) {
          ...
          if (pst) {
            ...
            html += `
              <td class="text-right rp5 ${label_cls}">${label}</td>
              <!-- PATCH: SEC->AUX in override (string) -->
              <td class="text-bold text-right rp5">${
                pst.override ? secToAux(pst.override) : pst.value
              }</td>
              <!-- PATCH: SEC->AUX in name -->
              <td class="lp5">${secToAux(name)}</td>`;
...
// Old rad presets using pri and sec
// var Presets = function(data, unit) {
...
//     // changes SEC to AUX
//     priorities = priorities.map(p => (p === "SEC" ? "AUX" : p));
...
var WaypointsCondensed = function (data, unit) {
  var data = data;
  var unit = unit;
  ...
  // Keep your existing break logic style (optional).
  // If you don't need it, you can delete on_break entirely.
  this.on_break = function () {
    var poi = sections["poi"];
    var sequence = sections["sequence"];

    if (sequence && sequence.content && sequence.content.length) {
      sequence.force_newpage_after = true;
    } else if (poi && poi.content && poi.content.length) {
  ...
  this.table = function () {
    // IMPORTANT: no "kb-height" and no forced big table layout
    var table = $(`
      <table class="kb-width std tbl-waypoints-condensed" style="table-layout: fixed;">
        <colgroup>
          <col style="width:40${unit}" />
          <col style="width:150${unit}" />
          <col />
          <col style="width:95${unit}" />
        </colgroup>
        <tbody>
          <tr class="header">
            <th>ID</th>
  ...
  this.content = (function () {
    ...
    for (var wp of data.waypoint.waypoints) {
      // Only show rows that actually have something meaningful
      let [lat, lon] = display_coord(data, wp, true);

      // Skip completely empty waypoints
      if (!wp.typ && !wp.name && lat === "" && lon === "") {
        continue;
      }

      // Build LAT/LON display (two-line if both exist)
      let latlon = "";
      if (lat && lon) {
        latlon = `${lat}<br>${lon}`;
      } else if (lat) {
      ...
      // Bulls calculation (only if bulls + wp coords exist)
      let bulls = "";
      if (
        data.waypoint.bullseye &&
        data.waypoint.bullseye.lat &&
        data.waypoint.bullseye.lon &&
        wp.lat &&
        wp.lon
      ) {
        let [brg, rng] = get_brg(
          ...
          wp.lat,
          wp.lon,
          data.waypoint["gs-units"] == "kmh",
        );
        bulls = `${brg}/${rng.padStart(3)}`;
      }

      let html = `
        <tr class="wp-border-top">
          <td class="text-center">${wp.typ || ""}</td>
          <td class="text-center text-bold" style="white-space: pre-line;">${
            latlon || ""
          }</td>
          <td class="lp5" style="white-space: normal;">${wp.name || ""}</td>
          <td class="text-center" style="white-space: pre;">${bulls}</td>
        </tr>
      `;
  ...
  // KEY: DO NOTHING. This prevents the table being padded to full page height.
    ...
    for (var wp of data.waypoint.waypoints) {
      ...
      if (wp.name !== "" || wp.act !== "" || !(lat === "" && lon === "")) {
        ...
        html += `
            ...
            <td class="lp5" style="white-space: normal" rowspan=2>${
              wp.name
            }</td>
            <td class="text-left lp5">${alt_formatter(data, wp.alt | "")}</td>
            <td class="text-left lp5">${
              wp.tbrg ? wp.tbrg.padStart(3, "0") : ""
            }</td>
        `;
        ...
        if (row === 0) {
          ...
          if (swing && wp.lat && wp.lon) {
            ...
            html += `
              ...
              <td class="text-center" style='white-space: pre'>${brg}/${rng.padStart(
                3,
              )}</td>`;
          } else {
            html += `
        ...
        if (data.flight["flight-airframe"] === "F-14B" && wp.nearest_tcn) {
          html += `
            ...
            <td class="text-center" style='white-space: pre'>${wp.nearest_tcn.brg
              .toFixed(0)
              .padStart(3, "0")}/${wp.nearest_tcn.rng
              .toFixed(0)
              .padStart(3, " ")}</td>`;
        } else {
        ...
        content.push(
          $(
            `<tr class="wp-border-top" style="height:23px"><td class="text-center">${wp.typ}</td><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td></td></tr>`,
          ),
        );
      }
    ...
    for (var table of tables) {
      if (table[0] == elem) {
        ...
        while (extension > 0) {
          ...
          var row = $(
            `<tr class="${cls}"><td>&nbsp;</td><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td></td></tr>`,
          );
          body.append(row);
...
var Page = function (data, unit, id) {
  ...
  this.page = $(`
    <div id="page${this.id}" style="position: absolute; top: ${
      (id - 1) * 1200
    }px; height:100%; width: 780px">
  ...
  this.expand = function () {
    var single_page_expansions = ["notes", "waypoints"];
...
function RAMROD(data, unit) {
  ...
  this.table = function () {
    ...
    // Put ramrod back in - Jan 28th 2024
    // Got rid of ramrod  add to second div to re insert ramrod width: 529${unit}
    html = `
...
function DepArr(data, unit) {
  ...
  this.content = (function () {
    ...
    ["dep", "arr", "alt"].forEach((x) => {
      ...
      // Empty ? skip it
      var nonempty = Object.values(elem).filter(function (x) {
        return x && x != "" && x != "----";
      });
      if (!nonempty.length) {
      ...
      for (const agency of ["atis", "twr", "uhf", "", "gnd", "ctrl", "vhf"]) {
        ...
        if (elem[agency]) {
          html += `
            ...
            <td class="text-center lb0 text-pst text-bottom">${
              elem[agency].pst || ""
            }</td>`;
        } else {
    ...
    for (elem of data.comms.agencies) {
      ...
      if (elem.pri) {
        if (elem.pri.value.startsWith("MIDS")) {
          ...
          elem_html += `
            <td class="text-right rp2 text-bold rb0 align-top">${
              elem.pri.value
            }</td>
            <td class="text-left lb0 pt3 text-pst align-top">${
              elem.pri.pst || ""
            }</td>`;
        }
      } else {
...
function Notes(data, unit) {
  ...
  //   var elems = [];
  //   var root = $(`<div>${data["html"]}</div>`);

  //   for (var child of root.contents()) {
  //     // Handle top-level Page Break comments
  //     if (
  //       child.nodeType == Node.COMMENT_NODE &&
  //       child.data.trim() == "pagebreak"
  //     ) {
  //       elems.push(new PageBreak());
  //       continue;
  //     }

  //     // PATCH: Handle pagebreak comments nested inside elements (most commonly inside <p>...</p>)
  //     if (child.nodeType == Node.ELEMENT_NODE) {
  //       var $child = $(child);
  //       var html = $child.html() ?? "";

  //       // Split the element on any <!-- pagebreak --> / <!--pagebreak--> (case-insensitive)
  //       if (/<!--\s*pagebreak\s*-->/i.test(html)) {
  //         var parts = html.split(/<!--\s*pagebreak\s*-->/i);

  //         for (let i = 0; i < parts.length; i++) {
  //           // clone the element (keeps <p> styles/classes), then replace inner content with this segment
  //           var seg = $child.clone();
  //           seg.empty();
  //           seg.append(parts[i]); // keep HTML (br/img/etc)
  //           elems.push(seg);
  ...
  //   //     var ratio =
  //   //       Array.from(cols).reduce(
  //   //         (x, y) => x + parseFloat($(y).css("width")),
  //   //         0
  //   //       ) / 100;
  ...
  this.content = (function () {
    ...
    // helper: ensure everything is either PageBreak() or a jQuery object
    function pushElem(x) {
      if (!x) return;
      if (x instanceof PageBreak) {
        elems.push(x);
        return;
      }
      if (x.jquery) {
    ...
    for (var child of root.contents()) {
      ...
      if (
        ...
        pushElem(new PageBreak());
        continue;
      }

      // If a child is a table, duplicate it per row (original behavior)
      if (child.nodeType === Node.ELEMENT_NODE && child.tagName === "TABLE") {
        ...
        var css_w = ($tbl.css("width") || "").replace(/\s*%/, "");
        if (parseFloat(css_w) > 90) {
          $tbl.css("width", "100%");
        }

        // Fix colgroup ratios (original behavior)
        var cols = $tbl.find("colgroup > col");
        var ratio =
          Array.from(cols).reduce(
            (x, y) => x + parseFloat($(y).css("width") || 0),
            0,
          ) / 100 || 1;

        cols.each(function (idx, col) {
          var w = parseFloat($(col).css("width") || 0) / ratio;
          var evens = 100 / cols.length;

          if (w > evens - 1 && w < evens + 1) w = evens;
          $(col).css("width", w.toFixed(3) + "%");
        });

        $tbl.find("tbody > tr").each(function (idx, row) {
          var nt = $(child.cloneNode(true));
          var tb = nt.find("tbody");
          if (!tb.length) {
            tb = $("<tbody></tbody>");
          ...
          // TD width cleanup (original behavior)
          var tds = $(row).find("> td");
          var evens = 100 / (tds.length || 1);
          tds.each(function (idx, td) {
            var w = ($(td).css("width") || "").replace(/\s*%/, "");
            if (parseFloat(w) > evens - 1 && parseFloat(w) < evens + 1) {
              $(td).css("width", evens.toFixed(2) + "%");
            }
            $(td).css("height", "");
          });
          ...
          // If we are 100% then force page break
          if (nt.css("height") === "100%") nt.css("height", "");
      ...
      // PATCH: Handle pagebreak comments nested inside elements (commonly inside <p>...</p>)
      if (child.nodeType === Node.ELEMENT_NODE) {
        var $child = $(child);
        var html = $child.html() ?? "";
        ...
        if (/<!--\s*pagebreak\s*-->/i.test(html)) {
          ...
          for (let i = 0; i < parts.length; i++) {
            ...
            // Avoid emitting totally empty segments that create blank pages
            if (
              seg.text().trim() !== "" ||
              seg.find("img,table").length > 0 ||
              seg.html().includes("&nbsp;")
            ) {
              pushElem(seg);
            }
...
function Builder(data, unit) {
  ...
  sections = {
    ...
    waypoints_condensed: new WaypointsCondensed(data, unit),
  ...
  $(document).on(
    "RowRepeat",
    function () {
      this.row_complete(false);
    }.bind(this),
  );
  $(document).on(
    "RowComplete",
    function () {
      this.row_complete(true);
    }.bind(this),
  );
  $(document).on(
    "SectionComplete",
    ...
    for (var img of images) {
      ...
      if (p.prop("tagName") == "TD" && !p.hasClass("body")) {
        ...
        if (previous.length > 1) {
          ...
          previous.length == 1 &&
          previous[0].nodeType == Node.ELEMENT_NODE
        ) {
      ...
      if (ir > pr) {
        ...
        $(img).css(
          "padding-left",
          (tw - (th / img.naturalHeight) * img.naturalWidth) / 2,
        );
      }
    }
  };
  ...
  this.complete = function () {
    ...
    $("body").append(
      $(`<div style="display:none" id="page_count">${pages.length}</div>`),
    );
  };
  ...
  this.process_section = function () {
    ...
    if (body && row) {
      ...
      // // Forced Page Break, we do this here so the BR cleanup above will have happened
      ...
      //   // ALWAYS force a new page when we hit a pagebreak.
      //   // The old logic ignored breaks when they landed at the top of a new page,
      //   // which is common after a split/migration.
      //   page = page.next();
      //   pages.push(page);

      //   // Re-add section header on the new page (keep the same behavior as a normal break)
      //   page.append($(`<div style="height:5${unit}"></div>`));
      //   page.append(header);

      //   // Ensure subsequent rows go into the new header's body
      //   body = header.find(".body");
      //   if (!body[0]) {
      //     body = header.find("tbody").last();
      //   }

      //   new_section = false; // header already added
      //   $(document).trigger("RowComplete");
      //   return;
      // }
      ...
      if (row.prop("tagName") == "TABLE") {
        ...
        if (
          ...
          parseFloat(row[0].style.width) > 90
        ) {
          row.css("width", "100%");
